...
class VisualizationUnionMask(object):
    def __init__(self, cfg, instance_mode=ColorMode.IMAGE, parallel=False, conf_thres=0.5):
        """
        Args:
            cfg (CfgNode):
    ...
    def run_on_video(self, frames, audio_feats, test_data):
        """
        Args:
            frames (List[np.ndarray]): a list of images of shape (H, W, C) (in BGR order).
                This is the format used by OpenCV.
        ...
        """
        vis_output = None
        gt = test_data["instances"]

        v_input = {"frames": frames, "audio_feats": audio_feats, "instances": gt}
        predictions = self.predictor(v_input)

        image_size = predictions["image_size"]
        _union_frame_masks = predictions["union_frame_masks"]

        # pred_masks = []
        # for m in _union_frame_masks:
        #     pred_masks.append(m)

        frame_masks = list(_union_frame_masks)
        total_vis_output = []
        ...
        for frame_idx in range(len(frames)):
            ...
            # ins.pred_masks = torch.stack(frame_masks[frame_idx], dim=0)
            ins.pred_masks = frame_masks[frame_idx]

            # ins.scores = torch.tensor([1.0])
            num_inst = len(frame_masks[frame_idx]) 
            ins.pred_classes = torch.zeros(num_inst, dtype=torch.int64)
...
class VideoPredictor(DefaultPredictor):
    ...
    def __call__(self, v_input):
        ...
        with torch.no_grad():  # https://github.com/sphinx-doc/sphinx/issues/4258
            ...
            gt = None        
            if "instances" in v_input.keys():
                gt = v_input["instances"]
            ...
            inputs = {"image": input_frames, "height": height, "width": width, "audio": audio_feats, "audio_mel": audio_feats, "instances": gt}
