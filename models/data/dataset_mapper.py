...
class AVISDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        audio_mels = dataset_dict.pop("audio_mel", None)
        
        instance_cnts = dataset_dict.pop("instance_cnt", None)

        if self.is_train or self.need_gt:
            _ids = set()
        ...
        dataset_dict["audio"] = []
        dataset_dict["audio_mel"] = []
        dataset_dict["frame_idx"] = list(selected_idx)
        dataset_dict["instance_cnt"] = []
        for frame_idx in selected_idx:
            dataset_dict["file_names"].append(file_names[frame_idx])
            dataset_dict["audio"].append(audio_feats[frame_idx])
            dataset_dict["audio_mel"].append(audio_mels[frame_idx])
            dataset_dict["instance_cnt"].append(instance_cnts[frame_idx])
            ...
            if (video_annos is None) or (not self.is_train and not self.need_gt):
                continue
