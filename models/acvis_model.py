    ...
    def inference(self, batched_inputs):
        ...
        with torch.no_grad():
            for i in range(math.ceil(num_frames / self.test_run_chunk_size)):
                ...
                sound_flags.append(sound_token.argmax(dim=-1).long().reshape(-1))
        ...
        gt_sound_flags = torch.clamp(gt_instance_cnt, max=2).long()

        correct_sound_frame = (sound_flags == gt_sound_flags).sum().item()
        total_frame = len(gt_sound_flags)


        total_per_cls = torch.bincount(gt_sound_flags, minlength=3)

        correct_mask = (sound_flags == gt_sound_flags)
        correct_per_cls = torch.bincount(gt_sound_flags[correct_mask], minlength=3)
