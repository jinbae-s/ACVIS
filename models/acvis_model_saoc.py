...
def saoc_loss(logits: torch.Tensor, y: torch.Tensor, num_classes: int) -> torch.Tensor:
    y = y.to(torch.int64)

    num_examples = 0
    losses = logits.new_tensor(0.)
    
    for i in range(num_classes - 1):
        mask = (y > (i - 1))
        if mask.any():
            target = (y[mask] > i).to(torch.int64)
            pred = logits[mask, i]
            
            loss_i = -torch.sum(F.logsigmoid(pred)*target + (F.logsigmoid(pred)-pred)*(1-target))
            losses += loss_i
            num_examples += target.numel()

    return losses / max(num_examples, 1)

@META_ARCH_REGISTRY.register()
class ACVIS_SAOC(nn.Module):
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.audio_backbone = torch.hub.load('harritaylor/torchvggish', 'vggish', pretrained=True)
        self.audio_backbone.preprocess = False
        ...
        # self.cnt_criterion = nn.BCEWithLogitsLoss()
        # self.cnt_criterion = nn.CrossEntropyLoss()
        self.cnt_criterion = saoc_loss
    ...
    def train_model(self, batched_inputs):
        ...
        for video in batched_inputs:
            ...
            for audio in video["audio_mel"]:
                audios.append(torch.tensor(audio).to(self.device))

        audios = torch.stack(audios).unsqueeze(1).float()

        audio_features = self.audio_backbone(audios)
        ...
        outputs, frame_queries, mask_features, instance_token = self.sem_seg_head(image_features, audio_features)

        K = instance_token.shape[-1] + 1

        gt_instance_cnt = torch.clamp(torch.tensor(
            [video["instance_cnt"] for video in batched_inputs],
            device=self.device
        ), max=K-1).long()

        INSTANCE_CNT_WEIGHT = 1.0
        # loss_instance_cnt = {"loss_instance_cnt": INSTANCE_CNT_WEIGHT * self.cnt_criterion(instance_token.reshape(-1,C), gt_instance_cnt.reshape(-1))}

        loss_cnt = self.cnt_criterion(
            logits=instance_token.reshape(-1, K-1),
            y=gt_instance_cnt.reshape(-1),
            num_classes=K,
        )
        loss_instance_cnt = {"loss_instance_cnt": INSTANCE_CNT_WEIGHT * loss_cnt}
        ...
        acvis_outputs = self.vita_module(frame_queries, audio_features)
        acvis_outputs["pred_masks"] = torch.einsum("lbqc,btchw->lbqthw", acvis_outputs["pred_mask_embed"], mask_features)
        for out in acvis_outputs["aux_outputs"]:
        ...
        for k in list(losses.keys()):
            if k in self.criterion.weight_dict:
                ...
                losses.pop(k)
        avism_loss_dict = self.vita_criterion(acvis_outputs, clip_targets, frame_targets, fg_indices)
        ...
        losses.update(loss_instance_cnt)
    ...
    def inference(self, batched_inputs):
        ...
        audios = torch.tensor(batched_inputs["audio_mel"]).to(self.device).unsqueeze(1).float()
        audio_features = []
        sound_flags = []
        ...
        with torch.no_grad():
            for i in range(math.ceil(num_frames / self.test_run_chunk_size)):
                ...
                audios_chunk = audios[i*self.test_run_chunk_size : (i+1)*self.test_run_chunk_size]

                features = self.backbone(images.tensor)
                audio_features_chunk = self.audio_backbone(audios_chunk)
                outputs, _frame_queries, _mask_features, sound_token = self.sem_seg_head(features, audio_features_chunk)

                # sound_token: (T', K-1)
                probs = torch.sigmoid(sound_token)     # (T', K-1), p_i = P(y > i | x, via logistic)
                q = torch.cumprod(probs, dim=-1)       # (T', K-1), q_i = Π_{j<=i} p_j = P(y > i)

                K = sound_token.shape[-1] + 1
                parts = []

                # P(y = 0) = 1 - q_0
                parts.append((1 - q[..., 0]).unsqueeze(-1))

                # P(y = c) = q_{c-1} - q_c,  for c = 1 .. K-2
                for c in range(1, K-1):
                    parts.append((q[..., c-1] - q[..., c]).unsqueeze(-1))

                # P(y = K-1) = q_{K-2}
                parts.append(q[..., -1].unsqueeze(-1))

                class_probs = torch.cat(parts, dim=-1)   # (T', K)
                pred_counts = class_probs.argmax(dim=-1) # (T',)

                sound_flags.append(pred_counts.long().reshape(-1))
                ...
                if audio_features_chunk.dim() == 1:          # (128,) → (1,128)
                    audio_features_chunk = audio_features_chunk.unsqueeze(0)
                audio_features.append(audio_features_chunk)
        ...
        sound_flags = torch.cat(sound_flags) #.squeeze()
        if "instance_cnt" in batched_inputs.keys():
            gt_instance_cnt = torch.tensor(batched_inputs["instance_cnt"], device=self.device)
            gt_sound_flags = torch.clamp(gt_instance_cnt, max=K-1).long()

            correct_sound_frame = (sound_flags == gt_sound_flags).sum().item()
            total_frame = len(gt_sound_flags)


            total_per_cls = torch.bincount(gt_sound_flags, minlength=K)

            correct_mask = (sound_flags == gt_sound_flags)
            correct_per_cls = torch.bincount(gt_sound_flags[correct_mask], minlength=K)
        else:
            correct_sound_frame = 0
            total_frame = 0
            correct_per_cls = torch.zeros(3)
            total_per_cls = torch.zeros(3)

        # acc_per_cls = (correct_per_cls.float() / total_per_cls.clamp(min=1).float())

        # cm = torch.bincount(gt_sound_flags * K + sound_flags, minlength=K*K).reshape(K, K)
        ...
        mask_cls = acvis_outputs["pred_logits"][-1, 0]       # cQ, K+1
        mask_embed = acvis_outputs["pred_mask_embed"][-1, 0] # cQ, C
        ...
        confidence = 0.3
        indices = torch.nonzero(scores_per_video > confidence).squeeze(-1).cpu()
        ...
        processed_results = {
            ...
            "correct_sound_frames": correct_sound_frame,
            "total_frames": total_frame,

            "correct_sound_frames_per_class": correct_per_cls.tolist(),
            "total_frames_per_class": total_per_cls.tolist(),
        }
